...
**Signetry is the platform. Models/agents are pluggable executors.** It competes with
...
*propose* a change; only Signetry may *admit* authority and seal a receipt. The hard
...
|---|---|---|
| **Policy** | Capability graph: paths, diff budget, required checks, and (v2) tool / bash / MCP / skill allowlists. Fail-closed; policy only restricts. | `signetry-core` (`contract.py`) |
| **Enforcement** | Three choke points — soft guard (editor hook), hard `signetry admit` (CLI/MCP/API), hardest `signetry-action` required check. Plus **plan binding** (CaMeL/DRIFT): a `PlanCapabilitySet` frozen from mission + contract before the executor runs; deviations cap authority. | `signetry-core`, `signetry-action`, plugins |
| **Verification** | The writer never self-approves. A deterministic path (contract, secrets, checks) **and** an independent masked re-check (MELON) that raises a *hijack signal* when a change does what an injection surface pushed for. | `signetry-core` (`verifier.py`) |
| **Authority** | Earned, revocable, run-bound ladder: `L0 observe` / `L1 analyze` / `L2 branch-PR`. Passport binds repo + receipt hash + base commit + executor/config hash + expiry, with an emergency brake. `auto_merge` always false. | `signetry-core` (`passport.py`) |
| **Proof** | **G1** capability integrity · **G2** behavioral authenticity · **G3** interaction auditability. Ed25519 receipt → in-toto/SLSA export → Merkle transparency log. | `signetry-core` (`receipt.py`, `gates.py`, `provenance.py`, `transparency.py`) |
| **Supply chain** | **Admitted Extension**: fingerprint a skill/MCP extension's bytes, quarantine its docs/tool descriptions before ingest, admit/deny against the allowlist, emit a CycloneDX ASBOM. | `signetry-core` (`extension.py`) |
| **Product UI** | A production hosted console (sign-in, multi-repo, admission detail, passports, brake, receipts/audit) rendering the same Admission Decision Pack. | `signetry` (hosted) |
...
a subset to outputs + a PR comment (rendered by `signetry comment` in the kernel, so
...
```
load CapabilityContract (.signetry/admission.yaml v2)
...
- Governance logic forked outside `signetry-core`
