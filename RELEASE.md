...
Umbra is not a monorepo — each repository releases independently — but governance
capabilities flow in **one direction**, so releases follow a fixed order.

## The order

```
umbra-core  →  integrations (action / plugins / app)  →  hosted (umbra)
   (kernel)          (pin the new umbra-core)              (deploy)
```
...
3. **Hosted `umbra`** deploys last, consuming the released `umbra-core` and
...
- All repos use [SemVer](https://semver.org/). Until `umbra-core` reaches `1.0.0`,
...
- Governance logic lives only in `umbra-core`; no integration forks it.
- A capability is available on a surface only after the `umbra-core` release that
  provides it is published and pinned.

## Checklist (per umbra-core release)
