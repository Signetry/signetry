...
Open source (MIT), on PyPI (`pip install umbra-core`) and the GitHub Marketplace.
Contributions welcome — 10 tagged good-first-issues.
...
- **Kernel (PyPI `umbra-core`):** https://github.com/bkd-dotcom/umbra-core
...
Each has `CONTRIBUTING.md` + `CODE_OF_CONDUCT.md`. Ground rules: governance stays in
`umbra-core`; SAFE cases must stay 0-FP; every change ships with a test and passes CI.
